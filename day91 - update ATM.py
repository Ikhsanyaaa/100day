...
def setoran():
    ...
    pil = str(input("apakah anda ingin mencetak struk? (y/t) : "))
    if pil == "t" : 
        pass
    elif pil == "y" : 
        struk = open("struk.txt","x")
        teks = f'''===struk setoran===
nama : {username}
jumlah setoran : Rp.{setor}
jumlah saldo saat ini : Rp.{saldo}
===terimakasih==='''
        struk = open("struk.txt","w")
        struk.write(teks)
        struk.close()
        print("struk berhasil dibuat!")
    ulang()
